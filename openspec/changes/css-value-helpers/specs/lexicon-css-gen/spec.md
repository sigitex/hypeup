...
#### Scenario: JS reserved word property uses $ suffix
- **WHEN** `css.gen.ts` is inspected for the `continue` property
- **THEN** it SHALL be declared as `continue$` (not `_continue`)
