...
#### Scenario: Extension symbols included in pre-scan
- **WHEN** extensions define symbols `fs` and `m4`
- **THEN** the pre-scan identifier set SHALL include `fs` and `m4` so files containing them are not skipped
